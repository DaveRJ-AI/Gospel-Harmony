...
const REGION_ANCHORS: Record<string, { x: number; y: number }> = {
  ...
  judea: { x: 700, y: 620 },
  perea: { x: 920, y: 450 },
  transjordan: { x: 980, y: 330 },
  "judean-wilderness": { x: 940, y: 660 },
  decapolis: { x: 1040, y: 250 },
  default: { x: 600, y: 410 },
};
...
function getNodeClassNames(
  ...
  if (selected) return "fill-amber-300 stroke-amber-700";
  if (highlighted) return "fill-sky-300 stroke-sky-700";
...
function buildRelationalLayout(nodes: PlaceWithOverlay[]): PositionedNode[] {
  ...
  for (const [groupKey, groupNodes] of grouped.entries()) {
    const anchor = REGION_ANCHORS[groupKey] ?? REGION_ANCHORS.default;
    const offsets = REGION_LAYOUT_OFFSETS[groupKey] ?? REGION_LAYOUT_OFFSETS.default;

    groupNodes.forEach((node, index) => {
      const offset = offsets[index % offsets.length];
      const layer = Math.floor(index / offsets.length);
      const spiralShift = layer * 24;

      positioned.push({
        ...node,
        x: anchor.x + offset.x + spiralShift,
        y: anchor.y + offset.y + spiralShift * 0.35,
      });
    });
  }
...
function buildModernLayout(nodes: PlaceWithOverlay[]): PositionedNode[] {
  ...
  if (withCoords.length === 0) return buildRelationalLayout(nodes);
  ...
  return nodes.map((node) => {
    if (!node.modernOverlay) {
      const fallback = REGION_ANCHORS[node.region ?? node.id] ?? REGION_ANCHORS.default;
    ...
    const x =
      padding +
      ((longitude - minLng) / Math.max(maxLng - minLng, 0.0001)) * innerWidth;
...
export default function MapView() {
  ...
  const nodes = useMemo<PositionedNode[]>(() => {
    return useModernOverlay
      ? buildModernLayout(graph.nodes)
      : buildRelationalLayout(graph.nodes);
  ...
  const travelPathsByGospel = useMemo(() => {
    ...
    for (const gospel of enabledGospels) grouped.set(gospel, []);
  ...
  return (
    <div className="w-full">
      <div className="mb-4 rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200">
        <h2 className="text-2xl font-semibold text-slate-900">Gospel Map</h2>
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
        <aside className="space-y-3 xl:sticky xl:top-4 xl:self-start">
          ...
          <ControlSection title="Gospel travel sequences">
          ...
          <ControlSection title="Selected place">
            {!selectedPlace ? (
              ...
              <div className="space-y-1.5 text-sm text-slate-700">
                ...
                <div>
                  <span className="font-semibold">Region:</span> {selectedPlace.region ?? "—"}
                </div>
                <div>
                  <span className="font-semibold">Certainty:</span> {selectedPlace.certainty}
                </div>
          ...
          <ControlSection title="Episodes at selected place">
        ...
        <main className="min-w-0">
          <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
            <div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 px-4 py-3">
              ...
              <div className="flex flex-wrap gap-2">
                {enabledGospels.map((gospel) => (
                  <div
                    key={gospel}
                    className="flex items-center gap-2 rounded-full bg-slate-50 px-3 py-1 text-xs text-slate-700 ring-1 ring-slate-200"
                  >
                    <span
            ...
            <div className="relative h-[820px] w-full overflow-auto bg-slate-50">
              <svg
                viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`}
                className="h-full min-w-[900px] w-full"
                role="img"
                aria-label="Gospel map visualization"
              >
                {useModernOverlay ? (
                  <>
                    <rect x={40} y={40} width={1120} height={740} className="fill-white" />
                    <text x={60} y={72} className="fill-slate-400 text-[14px]">
                      Approximate modern-coordinate projection
                    </text>
                  </>
                ) : (
                  <>
                    <text x={210} y={80} className="fill-slate-400 text-[16px]">Galilee</text>
                    <text x={500} y={320} className="fill-slate-400 text-[16px]">Samaria</text>
                    <text x={690} y={785} className="fill-slate-400 text-[16px]">Judea</text>
                    <text x={910} y={500} className="fill-slate-400 text-[16px]">Perea</text>
                    <text x={1030} y={210} className="fill-slate-400 text-[16px]">Decapolis</text>
                  </>
                )}
                  ...
                  graph.edges.map((edge) => {
                    ...
                    return (
                      <line
                        ...
                        strokeWidth={highlighted ? 2.8 : Math.max(1.2, edge.weight * 1.5)}
                        strokeDasharray={
                          edge.relationType.includes("travel") ||
                          edge.relationType.includes("sequence")
                            ? "7 5"
                            : undefined
                        }
                        opacity={0.95}
              ...
              {selectedEpisode && (
                <div className="absolute bottom-4 right-4 max-w-[320px] rounded-2xl border border-slate-200 bg-white p-4 shadow-lg">
                  <div className="mb-1 text-sm font-semibold text-slate-900">
                    {selectedEpisode.title}
                  </div>
                  <div className="mb-2 text-xs capitalize text-slate-500">
